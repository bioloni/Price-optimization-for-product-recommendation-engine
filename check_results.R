#comparison of means 
#uso el performance clean de trainer, antes de balancear clases
...
for (i in 1:rango){
  ...
  training1<-performance_clean[performance_clean$Product.Category..Enhanced.E.commerce.==ranges$Category[i],]
...
colnames(df2)<-c("Model","Category","Upper_Value","Gain_Revenue","Moderate_Lost_revenue","Pesimist_lost_revenue",
                 "Conv_Probability","Hidden_Buyers","Avg.SBIR_price","Avg.SDBOR_price")
...
df3<-df2[df2$net_revenue_pesimist>0,]
df4<-df2[df2$net_revenue_moderate>0,]
sum(df3$net_revenue_pesimist)
sum(df4$net_revenue_pesimist)
...
write.csv(df2,file="C://Users//escoloni//Desktop//grandvision//project_1//v4.5//test//revenue_estimate_for_bestparameters.csv")
